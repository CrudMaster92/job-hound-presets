...
class CatalogBuildTests(unittest.TestCase):
    ...
    def test_page_limits_and_multi_monitor_membership(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(fortune["company_count"], 250)
            self.assertEqual(fortune["monitor_count"], 236)
            ...
            self.assertEqual([page["count"] for page in detail["member_pages"]], [100, 100, 50])
            members = {"companies": [
            ...
            self.assertEqual(len({member["company_id"] for member in members["companies"]}), 250)
            bundle = json.loads((api / fortune["bundle_path"]).read_text(encoding="utf-8"))
            self.assertEqual(len(bundle["companies"]), 236)
            self.assertEqual(len(members["companies"]), 250)
            self.assertEqual(members["companies"][0]["rank"], 1)
            self.assertEqual(members["companies"][-1]["rank"], 250)
            self.assertEqual([m["rank"] for m in members["companies"]], [160 if rank == 161 else rank for rank in range(1, 251)])
