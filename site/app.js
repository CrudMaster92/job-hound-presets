...
function renderCompanies() {
  ...
  $('companies').innerHTML=rows.map(company=>`<article class="company-card" id="company-${esc(company.id)}"><header>${logo(company)}<span><h2>${esc(company.name)}</h2><p>${esc((company.facets?.industries||[]).map(title).join(' · ')||'Company')}</p></span></header><div class="tags">${(company.facets?.tags||[]).slice(0,4).map(tag=>`<span>${esc(tag)}</span>`).join('')}</div><p>${company.monitor_count||0} monitor${company.monitor_count===1?'':'s'}${company.adapters?.length?` · ${esc(company.adapters.map(title).join(' · '))}`:''}</p>${company.availability?.status==='unavailable'?`<p class="availability"><b>Temporarily unavailable</b>${esc(company.availability.message)}</p>`:`<button data-company="${esc(company.id)}">Choose monitors <span>+</span></button><div class="company-monitors"></div>`}</article>`).join('')||'<p class="empty">No companies match these filters.</p>';
